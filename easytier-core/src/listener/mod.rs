...
where
    ...
    H: AcceptedSocketHandler<Accepted> + ?Sized + 'static,
{
    pub fn new_with_events(handler: Arc<H>, events: Arc<dyn CoreEventSink>) -> Self {
        ...
        let initial_listeners = initial_listeners
            ...
            .map(|listener| {
                listener.map(|listener| {
                    RegisteredListener::new(listener, self.events.clone(), self.registry.clone())
                })
            })
            .collect::<Vec<_>>();
            ...
            None => {
                ...
                match listener.listen().await {
                    Ok(()) => {
                        ...
                        RegisteredListener::new(listener, events.clone(), registry.clone())
                    }
                    Err(error) => {
                        listen_error_count += 1;
                        let will_retry = listen_error_count <= options.max_listen_retries;
                        events.emit(CoreEvent::ListenerAddFailed {
                        ...
                        crate::foundation::time::sleep(options.listen_retry_delay).await;
                        continue;
                    }
                }
            }
        ...
        loop {
            ...
            let accepted = match listener.accept().await {
                ...
                Err(error) => {
                    ...
                    crate::foundation::time::sleep(options.accept_retry_delay).await;
                    break;
                }
            };
            ...
            accepted_tasks.spawn(async move {
                if let Err(error) = handler.handle_accepted_socket(accepted).await {
                    events.emit(CoreEvent::ListenerAcceptedSocketHandleFailed {
                        url: listener_url,
                        error: format!("{error:?}"),
                    });
                }
...
mod tests {
    ...
    async fn required_listener_reuses_successful_initial_listen() {
        ...
        assert!(
            events
                ...
                .unwrap()
                .iter()
                .any(|event| matches!(event, CoreEvent::ListenerAdded { .. }))
        );
    }
    ...
    async fn required_listener_failure_does_not_leave_partial_tasks_running() {
        ...
        assert!(
            events
                ...
                .unwrap()
                .iter()
                .all(|event| !matches!(event, CoreEvent::ListenerAdded { .. }))
        );
    }
    ...
    async fn stop_joins_in_flight_handler_tasks_and_is_one_shot() {
        ...
        crate::foundation::time::timeout(Duration::from_secs(1), drop_rx)
            .await
            .unwrap()
            .unwrap();
        assert!(manager.run().await.is_err());
    ...
    async fn stop_interrupts_required_listener_startup() {
        ...
        crate::foundation::time::timeout(Duration::from_secs(1), started.notified())
            .await
            .unwrap();
        manager.stop().await;
